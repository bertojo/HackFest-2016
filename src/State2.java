...
public class State2 {
	...
	public static void run(AvalonBot bot, Message message, TreeMap<Long, Game> games) {
		...
		bot.sendMessage("You are a bitch", message.getChatId());
		...
		for (Player player : game.players) {
			if (player.name.equals(message.getFrom().getFirstName())) {
				if (player.id != 0) {
					...
					bot.sendMessage("Established connection with " + player.name + " (" + game.pmChannelCount + "/" + game.players.size() + ")", game.gameId);
